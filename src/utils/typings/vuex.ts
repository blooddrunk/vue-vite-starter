type GetRestFuncType<T> = T extends (
  context: any,
  ...params: infer P
) => infer R
  ? (...args: P) => R
...
type AddPrefix<Keys, Prefix = ''> = `${Prefix & string}${Prefix extends ''
  ? ''
  : '/'}${Keys & string}`;

type GetMutationsTypes<Module, ModuleName = ''> = Module extends {
  mutations: infer M;
}
  ? {
      [MutationKey in keyof M as AddPrefix<
        MutationKey,
        ModuleName
      >]: GetRestFuncType<M[MutationKey]>;
    }
  : never;

type GetActionsTypes<Module, ModuleName = ''> = Module extends {
  actions: infer M;
}
  ? {
      [ActionKey in keyof M as AddPrefix<
        ActionKey,
        ModuleName
      >]: GetRestFuncType<M[ActionKey]>;
    }
  : never;
...
type GetSubModuleActionsTypes<Module> = Module extends {
  modules: infer SubModules;
}
...
export type GetMutationsType<R> = UnionToIntersection<
  GetSubModuleMutationsTypes<R> | GetMutationsTypes<R>
>;
...
export type GetReturnType<T, K extends keyof T> = ReturnType<
  GetTypeOfKey<T, K>
>;
